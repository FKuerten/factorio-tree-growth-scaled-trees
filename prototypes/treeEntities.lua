...
local function createTreeEntityHierarchyForTree(configuration, oldTree)
  ...
  for _, options in pairs(configuration) do
    ...
    for _, v in pairs(options.next) do
      tree_growth.core.registerUpgrade{base = newName, upgrade = baseName .. v.suffix, probability = v.probability, minDelay = v.minDelay, maxDelay = v.maxDelay, variations="id"}
    end
